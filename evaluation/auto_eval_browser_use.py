...
def auto_eval_by_gpt4v(process_dir, openai_client, api_model, img_num):
    print(f'--------------------- {process_dir} ---------------------')
    res_files = sorted(os.listdir(os.path.join(process_dir, 'screenshots')))
    ...
    # find task, answer
    try:
        with open(os.path.join(process_dir, 'task_info.json'), 'r') as f:
            task_info = json.load(f)
        task_content = task_info['task']
        history = task_info['history']['history']
        if history[-1]['result'][-1]['is_done']!=True or history[-1]['result'][-1]['success']!=True:
            print(f'Not find answer for {process_dir}, the task is not done')
            return 0
        answer_content = history[-1]['result'][-1]['extracted_content']
    except (FileNotFoundError, json.JSONDecodeError):
        print(f'Not find task_info.json for {process_dir}')
        return 0
    ...
    for png_file in end_files:
        b64_img = encode_image(os.path.join(process_dir, 'screenshots', png_file[0]))
