...
2. Web Page Markdown: This is the content cleaned from the source code of the web page, showing the current status of the web page. It serves as textual proof of the actions taken in response to the instruction.
...
def load_web_dump(process_dir):
    web_dump_folder = os.path.join(process_dir, 'web_dump')
    md_files = [f for f in os.listdir(web_dump_folder) if f.endswith('.md')]
    md_files.sort(key=lambda x: int(''.join(filter(str.isdigit, x))))
    last_md = md_files[-1]
    # print(last_md)
    with open(os.path.join(web_dump_folder, last_md), 'r', encoding='utf-8') as fr:
        md_content = fr.read()
    return md_content
...
def auto_eval_by_gpt4v(process_dir, openai_client, api_model, img_num):
    ...
    web_dump = load_web_dump(process_dir)
    ...
    user_prompt_tmp = user_prompt_tmp.replace('<markdown>', web_dump)
...
def draw_conclusion(results):
    ...
    # 判斷結論
    if counts[1] >= 2 or (counts[1] == 1 and counts[None] == 2):  # 大多數為 1
        return 1
    elif counts[0] >= 2 or (counts[0] == 1 and counts[None] == 2):  # 大多數為 0
        return 0
    else:  # 其他情況
        return None
...
def main():
    ...
    for root, dirs, files in os.walk(args.process_dir):
        for dir in dirs:
            if 'task' in dir:
                responses = []
                for _ in range(3):
                    file_dir = os.path.join(args.process_dir, dir)
                    response = auto_eval_by_gpt4v(file_dir, client, args.api_model, args.max_attached_imgs)
                    responses.append(response)
                    if responses.count(1)==2 or responses.count(0)==2:
                        break
                final_response = draw_conclusion(responses)
                evaluation_result.append((file_dir, final_response))
                
    # save
    with open(os.path.join('./evaluation', 'auto_eval_result.json'), 'w') as fw:
        json.dump(evaluation_result, fw, indent=4)

if __name__ == '__main__':
    main()

    # load_web_dump('results/20241118_14_47_06/taskYoutube--3')